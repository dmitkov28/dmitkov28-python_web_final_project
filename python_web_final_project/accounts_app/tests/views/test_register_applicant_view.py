...
class TestRegisterApplicantView(TestCase, CreateUserAndProfileMixin):
    ...
    def test_correct_form_is_shown(self):
        ...
        self.assertIsInstance(form_shown, RegisterApplicantForm)
    ...
    def test_valid_form_expect_correct_user_and_profile_to_be_created(self):
        self.client.post(reverse('register applicant'), data=self.VALID_REGISTER_DATA)
    ...
    def test_invalid_form_expect_user_not_to_be_created(self):
        self.client.post(reverse('register applicant'), data=self.INVALID_REGISTER_DATA)
