...
class TestCompanyRegisterForm(TestCase, CreateCompanyAndJobMixin):
    ...
    def test_form_with_valid_data_expect_form_to_be_valid(self):
        form = RegisterCompanyForm(data=self.VALID_REGISTER_FORM_DATA)
    ...
    def test_form_with_valid_data_expect_correct_profile_and_user_to_be_created(self):
        ...
        self.assertTrue(UserModel.objects.filter(email=self.VALID_REGISTER_FORM_DATA['email']))
        ...
        self.assertEqual(company.name, self.VALID_REGISTER_FORM_DATA['company_name'])
